...
for dir in */ ; do
	...
	pandoc -o "$dirname.4_6.pdf" $dirname/*.md \
		...
		-V geometry:"paperwidth=4in, paperheight=6in, inner=0.5in, outer=0.5in, top=0.6in, bottom=0.5in, bindingoffset=0.125in" \
		-V fontsize="10pt" \
...
#		-V sansfont="Helvetica Neue" \
	...
	pandoc -o "$dirname.55_85.pdf" $dirname/*.md \
		...
		-V classoption=twoside \
		-V header-includes="\usepackage{fontspec}\setsansfont{Helvetica}\usepackage{sectsty}\allsectionsfont{\sffamily}" \
		-V geometry:"paperwidth=5.5in, paperheight=8.5in, inner=0.5in, outer=0.5in, top=0.6in, bottom=0.5in, bindingoffset=0.125in" \
		-V fontsize="10pt" \
		-V mainfont="Times New Roman" \
		-V mainfontoptions:LetterSpace=0
#		-V sansfont="Helvetica Neue" \
